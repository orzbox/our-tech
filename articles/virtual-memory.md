...
1. （可选）如果转换过程中发现映射表中没有物理内存，那么操作系统就将它从硬盘加载到内存并修改映射表

![](../pics/vm-work1.png)

1. 计算机通过物理地址读取RAM并将数据返还给程序。
...
1. 页表入口PTE告诉你所需要的页在硬盘中

1. 硬件（CPU）生成一个Page Fault页错误

1. 硬件跳到Page Fault Handler页错误处理程序来处理
   1. 操作系统选择RAM中的一个页剔除写到硬盘中
   2. 如果是需要读取的页是脏页，就需要先将其写回到硬盘再读取
   3. 操作系统读取所需要的页到RAM
   4. 操作系统修改页表的映射

1. 操作系统跳转到发生页错误的地方开始执行（这一次就不会产生页错误了）
